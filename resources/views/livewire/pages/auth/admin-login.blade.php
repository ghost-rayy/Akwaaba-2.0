    ...
    public function login(): void
    {
        ...
        if ($user->role !== 'super_admin') {
            throw \Illuminate\Validation\ValidationException::withMessages([
                'form.email' => GuardSession::wrongPortalMessage($user->role, 'admin.login'),
            ]);
        }

        Auth::guard('admin')->login($user, $this->form->remember);
        Session::regenerate();
        session()->forget('url.intended');

        cookie()->queue(GuardSession::portalCookie('admin'));
...
<div>
    ...
    <div class="text-center mb-6">
        <h2 class="text-xl font-bold text-gray-900">Administrator Sign In</h2>
